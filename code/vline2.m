function hhh=vline(x,in1,in2,in3,in4)
...
if length(x)>1  % vector input
    ...
    if length(label)
        ...
        xunit=(x-xx(1))/xrange;
        if xunit<0.8
            text(x+0.01*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'),'FontSize',in3,'FontName',in4)
        else
            text(x-.05*xrange,y(1)+0.1*(y(2)-y(1)),label,'color',get(h,'color'),'FontSize',in3,'FontName',in4)
        end
    end     
