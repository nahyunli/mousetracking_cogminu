...
fn = 'HV';
cd(loc)
dirs = 'C:\Users\Minu Kim\Documents\GitHub\mouse_tracking\nahyun_project';

mode = "word";
exp = 1;

if mode == "arrow"
    x = readtable([dirs,filesep,'bin_added_arrow.xlsx']);
elseif mode == "location"
    x = readtable([dirs,filesep,'bin_added_location.xlsx']);
elseif mode == "word"
    x = readtable([dirs,filesep,'bin_added_word.xlsx']);
end
...
%
    ...
    counter=counter+s1;  
    clear bkIDX bload prac et ovACC dcheck pAngle2 orgXY wAngle2 
end    
...
if mode == "arrow"
    save([fn '_processed_arrow.mat'],'fiRaw','-v7.3')
elseif mode == "location"
    save([fn '_processed_location.mat'],'fiRaw','-v7.3')
elseif mode == "word"
    save([fn '_processed_word.mat'],'fiRaw','-v7.3')
end
...
if mode == "arrow"
    load("HV_processed_arrow.mat");
elseif mode == "location"
    load("HV_processed_location.mat");
elseif mode == "word"
    load("HV_processed_word.mat");
end


pix_1 = [0, 0];

for i = 1:size(fiRaw.id,1)
    disp('hi')
    vx=fiRaw.coordinates_x(i, 1:end);
    vy=fiRaw.coordinates_y(i, 1:end);
    vt=fiRaw.timestamp(i, 1:end);
    [vvx, vvy, vvt] = timespace33HPL_startbox_interp(vx, vy, vt);
    
    %fiRaw.newtime_x(i,1:size(vvx,2)) = vvx;
    %fiRaw.newtime_y(i,1:size(vvy,2)) = vvy;
    %fiRaw.newtime_t(i,1:size(vvt,2)) = vvt;
    
    % calculate euclidean diance
    for j = 1:size(vvx,2)
        
        % calculate euclidian distance between zero and a coordinate point
        % zero is ommitted in the equation
        distance = sqrt(vvx(j)^2 + vvy(j)^2); 
        if distance > 32
            fiRaw.new_IT(i,1) = vvt(j);
            fiRaw.new_MT(i,1) = fiRaw.RT(i) - vvt(j);
            break;
        end
    end
end
...
if mode == "arrow"
    save([fn '_processed_arrow_added.mat'],'fiRaw','-v7.3')
elseif mode == "location"
    save([fn '_processed_location_added.mat'],'fiRaw','-v7.3')
elseif mode == "word"
    save([fn '_processed_word_added.mat'],'fiRaw','-v7.3')
end
...
mode = "word"; % choose between location, word, arrow

if mode == "arrow"
    load("HV_processed_arrow_added.mat");
elseif mode == "location"
    load("HV_processed_location_added.mat");
elseif mode == "word"
    load("HV_processed_word_added.mat");
end
...
if mode == "arrow"
    writetable(sss,'C:\Users\Minu Kim\Documents\GitHub\mouse_tracking\nahyun_project\indiv_arrow.xlsx')
    %writetable(ssss,'grand.xlsx')
elseif mode == "location"
    writetable(sss,'C:\Users\Minu Kim\Documents\GitHub\mouse_tracking\nahyun_project\indiv_location.xlsx')
    %writetable(ssss,'grand.xlsx')
elseif mode == "word"
    writetable(sss,'C:\Users\Minu Kim\Documents\GitHub\mouse_tracking\nahyun_project\indiv_word.xlsx')
end
